...
async function getSubscription(customerID: string) {
    ...
    return subscription as StripeTypes.Subscription & {
        plan: StripeTypes.Plan;
    };
}
...
export async function getSubscriptionRenewDate() {
    try {
        ...
        const renewing = formatTimestamp(
...
export async function updateSubscriptionPlan(newPlan: BillingPlan) {
    try {
        ...
        const uniqueRegions = new Set(
            user.servers.map((server) => server.region)
        );
        Array.from(uniqueRegions).forEach((region) => {
            fetch(
                `${serversByRegion[region]}/reset-user-cache/${user.id}`,
                VM_FETCH_CONFIG
            );
        });
